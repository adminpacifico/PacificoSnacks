...
class HrPayslip(models.Model):
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        for contract in contracts:
            ...
            if loans_ids:
                ...
                for loans in loans_ids:
                    ...
                    if loans[1] == 'BONIFICACION':
                       ...
                       nameb = loans[0]
                       codeb = loans[1]
                    if loans[1] == 'BONIFICACION_NP':
                        amountbnp = amountbnp + loans[2]
                        inputbnp_type_id = loans[3]
                        namebnp = loans[0]
                        codebnp = loans[1]
                ...
                if not amountbnp == 0:
                    self.env['hr.payslip.input'].create({
                        ...
                        "amount": amountbnp,
                        "payslip_id": self.id,
                        "input_type_id": inputbnp_type_id,
                        "name_input": namebnp,
                        "code_input": codebnp,
                    })
                if not amountd == 0:
            ...
            if self.type_payslip_id.name == 'Liquidación de Contrato':
                date_before_from = date(self.date_from.year, 1, 1)
                date_month_next = self.date_from + relativedelta(months=1)
                date_before_to = date(date_month_next.year, date_month_next.month, 1) - relativedelta(days=1)
