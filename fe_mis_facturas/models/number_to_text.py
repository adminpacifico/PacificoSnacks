...
MONEDAS = (
    ...
    {'country': u'Estados Unidos', 'currency': 'USD', 'singular': u'DOLAR', 'plural': u'DOLARES', 'symbol': u'US$'},
...
class NumberToText(models.TransientModel):
    _name = 'number.to.text'
    _description = 'Convertir numero a texto'
...
#               moneda = filter(lambda x: x['currency'] == mi_moneda, MONEDAS).next()
                monedaf = filter(lambda x: x['currency'] == mi_moneda, MONEDAS)
                moneda = list(monedaf)[0]
                if int(number) == 1:
                    entero = moneda['singular']
                else:
                    entero = moneda['plural']
#                    entero = moneda[0]['plural']
